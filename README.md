...
2. 开启云编译选择页面：你的仓库 → **Settings** → **Pages** → Source 选 **GitHub Actions** → 保存。等待约1分钟后，访问 `https://{你的用户名}.github.io/{仓库名}/` 即可打开可视化编译页面。

3. 在编译页面上选择设备型号、勾选需要的插件，填写 GitHub Token，点击「触发编译」。

4. 也可以手动触发：你的仓库 → **Actions** → **Build_imm_高通410系列** → **Run workflow** → 选择设备型号 → 填写需要插件包 → Run workflow 开始编译
![编译教程](img/1.png)

5. 编译约⏱️ 1.5-2 小时，插件越多时间越长.
![编译过程](img/2.png)

6. 上面出现绿色对号，就代表编译成功了，你的仓库 → **releases** → 下载固件包
![下载固件包](img/3.png)
